...
from models.common import SELayer
from models.common import PW_Conv
from models.common import BottleneckMOB
from models.experimental import MixConv2d
from models.experimental import GhostBottleneck
...
class Model(nn.Module):
    ...
    def forward_once(self, x, profile=False):
        y, dt = [], []  # outputs
        for m in self.model:
            if m.f != -1:  # if not from previous layer  判断是否该层的输入是上层的输出，如果不是则得到需要的输出
                x = (
                    y[m.f]
            ...
            if profile:  # 是否输出推理的 参数 时间等信息 默认不输出
                try:
                    import thop
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(
        ...
        m = eval(m) if isinstance(m, str) else m  # object_detect_eval strings
        for j, a in enumerate(args):
            try:
                args[j] = (
                    eval(a) if isinstance(a, str) else a
                )  # object_detect_eval strings
            except:
                pass
        ...
        if m in [
            ...
            c2 = (
                make_divisible(c2 * gw, 8) if c2 != no else c2
            )  # 当c2 不等于 namber of output（检测头输出数）执行，使其为8的整数倍
